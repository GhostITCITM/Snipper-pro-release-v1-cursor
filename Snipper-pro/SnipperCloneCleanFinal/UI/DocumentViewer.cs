    ...
    public partial class DocumentViewer : Form
    {
        ...
        private void ProcessSnip()
        {
            try
            {
                ...
                // Convert selection from displayed (zoomed) image back to original page image pixels
                var selectionOnOriginal = new System.Drawing.Rectangle(
                ...
                // 1. Try native PDF text extraction when possible
                if (_currentDocument.Type == DocumentType.PDF &&
                    _currentSnipMode != SnipMode.Validation && _currentSnipMode != SnipMode.Exception)
                {
                    extractedValue = ExtractTextFromPdfRegion(
                    ...
                    if (!string.IsNullOrWhiteSpace(extractedValue))
                    {
                        ...
                        {
                            var matches = System.Text.RegularExpressions.Regex.Matches(extractedValue, @"-?\d[\d,\.]*")
                                              .Cast<System.Text.RegularExpressions.Match>()
                ...
                // 2. OCR fallback if PDF extraction failed or document is an image
                if (!success && _currentSnipMode != SnipMode.Validation && _currentSnipMode != SnipMode.Exception)
                {
                ...
                // 3. Handle special modes that bypass text extraction
                if (_currentSnipMode == SnipMode.Validation)
                {
                    extractedValue = "✓ VERIFIED";
                    success = true;
                }
                else if (_currentSnipMode == SnipMode.Exception)
                {
                    extractedValue = "✗ EXCEPTION";
                    success = true;
                }
                else if (_currentSnipMode == SnipMode.Table && success)
                {
                    // For table snips, optionally post-process into tab-separated columns if requested (retain OCR path for now)
                }
                else if (_currentSnipMode == SnipMode.Sum && success && extractedNumbers.Length > 0)
                {
                    try
                    {
                        var sum = extractedNumbers
                                  .Select(s => decimal.Parse(s.Replace("$", string.Empty).Replace(",", string.Empty)))
                                  .Sum();
                        extractedValue = sum.ToString("N2");
                    }
                    catch { /* ignore parse errors – keep raw numbers */ }
                }
                ...
                var args = new SnipAreaSelectedEventArgs
                {
                    ...
                    SelectedImage = CropImageFromDisplayed(displayedImage, _currentSelection),
                ...
                _statusLabel.Text = success ?
                    $"✓ {_currentSnipMode} snip completed" :
                    $"✗ {_currentSnipMode} snip failed - no text found";
        ...
        private string ExtractTextFromPdfRegion(string pdfPath, int pageIndex, System.Drawing.Rectangle rectOnImage)
        {
            ...
            try
            {
                ...
                double pixelToPoint = 72.0 / PDF_RENDER_DPI;
                double pageHeightPoints = _currentDocument.Pages[pageIndex].Height * pixelToPoint;

                double left = rectOnImage.Left * pixelToPoint;
                double right = rectOnImage.Right * pixelToPoint;
                double top = pageHeightPoints - rectOnImage.Top * pixelToPoint;
                double bottom = pageHeightPoints - rectOnImage.Bottom * pixelToPoint;

                int charCount = FPDFText_GetBoundedText(textPage, left, top, right, bottom, null, 0);
                if (charCount <= 0) return string.Empty;

                ushort[] buffer = new ushort[charCount];
                FPDFText_GetBoundedText(textPage, left, top, right, bottom, buffer, charCount);
            ...
            {
                Logger.Error($"FPDF text extraction failed: {ex.Message}", ex);
                return string.Empty;
            }
            finally
            {
        ...
        // PDFium native bindings for text extraction
        [System.Runtime.InteropServices.DllImport("pdfium.dll", EntryPoint = "FPDF_LoadDocument", CharSet = System.Runtime.InteropServices.CharSet.Ansi)]
        ...
        private static extern int FPDFText_GetBoundedText(
            IntPtr text_page,
            double left,
            double top,
            double right,
            double bottom,
            [System.Runtime.InteropServices.Out] ushort[] buffer,
            int buffer_length);
    }
