...
# Installation script for Snipper Pro Excel Add-in (64-bit)
...
# Configuration
$config = @{
    InstallPath = "C:\Program Files\SnipperPro"
...
$regPaths = @(
    "HKLM:\Software\Microsoft\Office\Excel\Addins\$($config.ProgId)",
    "HKLM:\Software\Classes\$($config.ProgId)",
    "HKLM:\Software\Classes\CLSID\$($config.ClsId)"
)
...
if (Test-Path $config.InstallPath) {
    ...
    $dllPath = Join-Path $config.InstallPath $config.DllName
    ...
    Remove-Item -Path "$($config.InstallPath)\*.tlb" -Force -ErrorAction SilentlyContinue
}
...
# Configure Excel add-in registry
Write-Step "Configuring Excel add-in registry..."
$regPath = "HKLM:\Software\Microsoft\Office\Excel\Addins\$($config.ProgId)"
...
$tests = @(
    ...
    @{ Path = "HKLM:\Software\Classes\CLSID\$($config.ClsId)"; Message = "COM registration exists" },
